...
export default class StepZilla extends Component {
  ...
  applyValidationFlagsToSteps() {
    this.props.steps.map((i, idx) => {
      if (this.props.dontValidate) {
        i.validated = true; // eslint-disable-line
      } else {
        ...
        i.validated = (typeof i.component.type === 'undefined' || // eslint-disable-line
  ...
  getNavStates(indx, length) { // eslint-disable-line
    const styles = [];
  ...
  jumpToStep(evt) {
    if (evt.target === undefined) {
      ...
      if (!this.props.stepsNavigation ||
        evt.target.value === this.state.compState) { // if stepsNavigation is turned off or user clicked on existing step again (on step 2 and clicked on 2 again) then ignore
        evt.preventDefault();
        evt.stopPropagation();
      ...
      this.abstractStepMoveAllowedToPromise(movingBack)
        .then((valid = true) => { // validation was a success (promise or sync validation).
          // In it was a Promise's resolve() then proceed will be undefined, so make it true. Or else 'proceed' will carry the true/false value from sync v
          proceed = valid;
  ...
  next() {
    this.abstractStepMoveAllowedToPromise()
      .then((proceed = true) => {
        // validation was a success (promise or sync validation). In it was a Promise's resolve() then proceed will be undefined, so make it true.
